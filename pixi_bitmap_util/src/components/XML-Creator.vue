    ...
    <div class="XML">
      <div class="xml-area">
        <div class="form-group">
          <label class="text-white" for="XML"> Result XML File</label>
          <textarea class="form-control XML" id="XML" v-model="XMLText" rows="33" cols="50"
                    style="font-size: 12pt"></textarea>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  watch: {
    ...
    XMLText: function () {
      if (this.XMLTextIsReady) {
        console.warn("XMLText is changed")
        console.log(this.XMLText.split("<char"))
      }
...
<font>
  ...
  <chars count="${this.$store.getters.framesArrLength + 2}">\n`
      ...
      this.$store.state.framesArr.forEach(({frame, sourceSize, spriteSourceSize}, index) => {
        ...
        if (!this.$store.state.isTrimmed) {
            this.xOffsetCurrent = 0
        } else {
        ...
        let row = `    <char id="${this.charCodeArr[index]}" x="${x}" y="${y}" width="${symbolWidth}" height="${symbolHeight}" xoffset="${this.xOffsetCurrent}" yoffset="${yoffset}" xadvance="${this.xadvanceCurrent}" /><!-- ${this.$store.state.inputSymbolsArr[index]} -->\n`
        this.XMLText += row
